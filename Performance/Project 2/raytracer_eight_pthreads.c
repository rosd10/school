...
void *render(void *pointer)
{
  ...
  int i,j;
  float stepy = viewplaney0;
  float deltax = (viewplanex1 - viewplanex0) / temp->width;
  float deltay = (viewplaney1 - viewplaney0) / temp->height;
  stepy += deltay*temp->start;
  
  float *viewer = (float[3]){0.0f, 0.0f, -7.0f};
  for (i = temp->start; i < temp->start+128; i++) {
    float stepx = viewplanex0;
    for (j = 0; j < temp->width; j++) {
      ...
      if (b > 255) { b = 255; }
      temp->pixmap[j+i*(int)temp->width] = (r << 16) | (g << 8) | (b);
      stepx += deltax;
    }
    stepy += deltay;
  }
...
int main(int a, char *args[])
{
  ...
  argument *toSend[numberOfThreads];
  ...
  for(i = 0; i < numberOfThreads; i++){
    toReturn[i] = 0;
    toSend[i] = malloc(sizeof(argument));

    toSend[i]->start = i*128;
    toSend[i]->width = 1024.0f;
    toSend[i]->height = 1024.0f;
    toSend[i]->pixmap = pixmap;

    toReturn[i] = pthread_create(&threads[i], NULL, render, (void*)toSend[i]);
