...
void mandelbrot(float width, float height, unsigned int *pixmap)
{
  ...
  arguments *toSend[numberOfThreads];

  for(i = 0; i < numberOfThreads; i++){
    toReturn[i] = 0;
    
    toSend[i] = malloc(sizeof(arguments));

    toSend[i]->start = i * 128.0f;
    toSend[i]->width = 1024.0f;
    toSend[i]->height = 1024.0f;
    toSend[i]->pixmap = pixmap;

    toReturn[i] = pthread_create(&threads[i], NULL, calculate, (void*)toSend[i]);
  }
